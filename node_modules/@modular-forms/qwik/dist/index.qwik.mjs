...
import { noSerialize, inlinedQrl, useLexicalScope, implicit$FirstArg, componentQrl, useVisibleTaskQrl, _jsxC, Slot, _jsxS, useStore } from "@builder.io/qwik";
...
function getFormDataValues(formData, formDataInfo = {}) {
  ...
  const values = [
    ...
    const getValue2 = () => booleans.includes(template) ? true : dates.includes(template) ? getDate() : files.includes(template) && typeof value2 !== "string" ? noSerialize(value2) : numbers.includes(template) ? /^-?\d*(\.\d+)?$/.test(value2) ? parseFloat(value2) : getDate().getTime() : value2;
...
function formActionQrl(action, arg2) {
  return globalActionQrl(/* @__PURE__ */ inlinedQrl(async (jsonData, event) => {
    const [action2, arg22] = useLexicalScope();
...
function valiFieldQrl(schema) {
  return /* @__PURE__ */ inlinedQrl(async (value2) => {
    const [schema2] = useLexicalScope();
...
function valiFormQrl(schema) {
  return /* @__PURE__ */ inlinedQrl(async (values) => {
    const [schema2] = useLexicalScope();
    const resolvedSchema = await schema2.resolve();
...
const Lifecycle = /* @__PURE__ */ componentQrl(/* @__PURE__ */ inlinedQrl((props) => {
  ...
  return /* @__PURE__ */ _jsxC(Slot, null, 3, "Pr_0");
...
function toCustomQrl(action, { on: mode }) {
  return /* @__PURE__ */ inlinedQrl((value2, event, element) => {
...
function toLowerCase(options) {
  return toCustomQrl(/* @__PURE__ */ inlinedQrl((value2) => value2 && value2.toLowerCase(), "toLowerCase_toCustom_DqH9OzaMLmQ"), options);
}
function toTrimmed(options) {
  return toCustomQrl(/* @__PURE__ */ inlinedQrl((value2) => value2 && value2.trim(), "toTrimmed_toCustom_eTBCNsyXS74"), options);
}
function toUpperCase(options) {
  return toCustomQrl(/* @__PURE__ */ inlinedQrl((value2) => value2 && value2.toUpperCase(), "toUpperCase_toCustom_jn0vB1n0Eyw"), options);
}
function customQrl(requirement, error) {
  return /* @__PURE__ */ inlinedQrl(async (value2) => {
    const [error2, requirement2] = useLexicalScope();
...
function email(error) {
  return /* @__PURE__ */ inlinedQrl((value2) => {
    const [error2] = useLexicalScope();
...
function mimeType(requirement, error) {
  ...
  return /* @__PURE__ */ inlinedQrl((value2) => {
    const [error2, mimeTypes2] = useLexicalScope();
    return value2 && (Array.isArray(value2) ? [
      ...value2
