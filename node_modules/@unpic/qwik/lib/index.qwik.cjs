...
const transform$j = ({ url: originalUrl, width, height, format }) => {
  const url = toUrl(originalUrl);
...
const transform$i = ({ url: originalUrl, width, height, format }) => {
  const url = toUrl(originalUrl);
  setParamIfDefined(url, "w", width, true, true);
  setParamIfDefined(url, "h", height, true, true);
...
const formatUrl$2 = ({ host, cloudName, assetType, deliveryType, signature, transformations = {}, version, id, format }) => {
  if (format) {
...
const transform$e = ({ url: originalUrl, width, height, format = "auto" }) => {
...
const transform$d = ({ url: originalUrl, width, height }) => {
  const url = toUrl(originalUrl);
  setParamIfDefined(url, "width", width, true, true);
  if (width && height) {
...
const transform$a = ({ url, width, cdn }) => {
...
const transform$9 = (params) => transform$a({ ...params, cdn: "nextjs" });
...
const transform$3 = ({ url: originalUrl, width, height, format = "auto" }) => {
...
const getTransformer = (cdn) => ({
  imgix: transform$i,
  contentful: transform$k,
  "builder.io": transform$j,
  shopify: transform$h,
  wordpress: transform$g,
  cloudinary: transform$f,
  bunny: transform$d,
  storyblok: transform$c,
  cloudflare: transform$e,
  vercel: transform$a,
  nextjs: transform$9,
  scene7: transform$8,
  "kontent.ai": transform$b,
  keycdn: transform$7,
  directus: transform$6,
  imageengine: transform$5,
  contentstack: transform$4,
  "cloudflare_images": transform$3,
  ipx: transform$2,
  netlify: transform$1,
  imagekit: transform
})[cdn];
...
function transformSourceProps({ media, type, ...props }) {
  ...
  sizes || (sizes = getSizes(width, layout));
...
const Source = /* @__PURE__ */ qwik.componentQrl(/* @__PURE__ */ qwik.inlinedQrl((props) => {
  return /* @__PURE__ */ qwik._jsxS("source", {
    ...transformSourceProps(props)
  }, null, 0, "54_0");
}, "Source_component_ZfsLCyEz0BY"));
exports.Image = Image;
