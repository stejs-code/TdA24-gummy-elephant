import { componentQrl, inlinedQrl, _restProps, useStylesScopedQrl, useSignal, useVisibleTaskQrl, useLexicalScope, useTaskQrl, _jsxS, _jsxQ, _fnSignal } from "@builder.io/qwik";
import { parsePhoneNumberWithError, ParseError, parsePhoneNumber, AsYouType, isValidPhoneNumber, isPossiblePhoneNumber } from "libphonenumber-js";
import { countries } from "country-list-json";
import { timezoneCityToCountry } from "./timezone-city-to-country.qwik.mjs";
import styles from "./input-phone.css.qwik.mjs";
const find = (value, key) => {
  ...
  return !value ? void 0 : countries.find(fn);
};
...
const findCountryByUserTimezone = () => {
  ...
  return countries.find(({ name }) => name === city || name === country);
};
const InputPhone = /* @__PURE__ */ componentQrl(/* @__PURE__ */ inlinedQrl((props) => {
  ...
  useStylesScopedQrl(/* @__PURE__ */ inlinedQrl(styles, "InputPhone_component_useStylesScoped_PrjnwMdfAfM"));
  const defaultCountry = props.countryCode === "auto" ? findCountryByUserTimezone() : find(props.countryCode, "code");
  const inputRefSignal = useSignal();
  const selectRefSignal = useSignal();
  const numberSignal = useSignal(props.value ?? "");
  const countrySignal = useSignal(defaultCountry);
  const outputSignal = useSignal(props.value ?? "");
  const handleCountryChange = /* @__PURE__ */ inlinedQrl((country) => {
  ...
  useVisibleTaskQrl(/* @__PURE__ */ inlinedQrl(() => {
    const [countrySignal2, handleCountryChange2, handleNumberChange2, handleValidChange2, outputSignal2] = useLexicalScope();
  ...
  useTaskQrl(/* @__PURE__ */ inlinedQrl(({ track }) => {
    const [countrySignal2, handleCountryChange2, handleNumberChange2, handleValidChange2, numberSignal2, outputSignal2, props2] = useLexicalScope();
    const phone = track(() => numberSignal2.value);
    if (!phone)
      return;
    try {
      const phoneNumber = parsePhoneNumberWithError(phone, countrySignal2.value?.code);
      if (!phoneNumber)
        return;
  ...
  useTaskQrl(/* @__PURE__ */ inlinedQrl(({ track }) => {
    ...
    if (!code)
      return;
    try {
      const phoneNumber = parsePhoneNumber(numberSignal2.value, code);
      if (!phoneNumber)
        return;
      if (numberSignal2.value.at(0) === "+") {
        ...
        if (!country)
          return;
        outputSignal2.value = new AsYouType().input(`${country.dial_code}${nationalNumber}`);
      } else
        outputSignal2.value = new AsYouType(code).input(numberSignal2.value);
      handleValidChange2(outputSignal2.value);
    } catch (error) {
      if (numberSignal2.value.at(0) === "+") {
        const country = find(code, "code");
  ...
  return /* @__PURE__ */ _jsxS("div", {
    ...
    children: [
      /* @__PURE__ */ _jsxQ("button", null, {
        "aria-label": _fnSignal((p0) => p0.value?.flag ? `Country of ${p0.value.name}, dial code ${p0.value.dial_code}` : `Select the country of your phone number`, [
          countrySignal
        ], "p0.value?.flag?`Country of ${p0.value.name}, dial code ${p0.value.dial_code}`:`Select the country of your phone number`"),
        tabIndex: -1
      }, _fnSignal((p0) => p0.value?.flag ? p0.value?.flag : `🌐`, [
        countrySignal
      ], "p0.value?.flag?p0.value?.flag:`🌐`"), 3, null),
      /* @__PURE__ */ _jsxQ("select", {
        ref: selectRefSignal
      }, {
        "aria-label": "Select the country of your phone number",
        onChange$: /* @__PURE__ */ inlinedQrl((_, { value }) => {
          const [countrySignal2, inputRefSignal2, selectRefSignal2] = useLexicalScope();
        ...
        title: _fnSignal((p0) => p0.value?.name, [
          countrySignal
        ], "p0.value?.name")
      }, [
        /* @__PURE__ */ _jsxQ("option", null, null, "Select the country's phone code", 3, null),
        countries.map(({ code, dial_code, name }) => /* @__PURE__ */ _jsxQ("option", {
      ...
      /* @__PURE__ */ _jsxS("input", {
        ...props1,
        onInput$: [
          /* @__PURE__ */ inlinedQrl((_, { value }) => {
            const [countrySignal2, numberSignal2] = useLexicalScope();
            numberSignal2.value = new AsYouType(countrySignal2.value?.code).input(value);
        ...
        placeholder: _fnSignal((p0) => p0.placeholder ?? "Phone number", [
          props
